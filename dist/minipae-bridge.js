...
export class MinipaeBridge {
    ...
    call(command, args) {
        const out = execFileSync(this.python, [this.bridgePath, command, JSON.stringify(args)], { encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
    ...
    recall(agentId) {
        const r = this.call('recall', { agent_id: agentId, relay: this.relay });
        return r.map((rec) => ({
            id: rec.id ?? rec.event_id ?? '',
            agentId: rec.agent_id,
            primitive: rec.primitive,
            visibility: 'private', // NIP-AE is private by definition
            text: rec.text,
