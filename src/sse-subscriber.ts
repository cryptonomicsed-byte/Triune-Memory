...
export interface SseSubscriberOptions {
  /** Base URL of the Omo-Koda2 kernel, e.g. http://localhost:8080 */
  kernelUrl: string;
  /** Fallback agentId used until the first AgentBorn event is observed. */
  fallbackAgentId?: string;
  fetchImpl?: typeof fetch;
  onEvent?: (raw: KernelEvent, memoryEventId: string | null) => void;
}
...
export function parseSseData(rawEvent: string): KernelEvent | null {
  ...
  try {
    return JSON.parse(payload) as KernelEvent;
  } catch {
    return null;
  }
}
...
export class TriuneSseSubscriber {
  ...
  async handleEvent(ev: KernelEvent): Promise<string | null> {
    switch (ev.type) {
      case 'agent_born': {
      ...
      case 'thought_sealed': {
        ...
        const rec = await this.memory.write(
          this.currentAgentId,
          'think',
          `intent_hash:${hash} hermetic_score:${ev.hermetic_score ?? ''}`,
          'public',
        );
        return rec.id;
      }
      case 'act_executed': {
        ...
        const rec = await this.memory.write(
          this.currentAgentId,
          'act',
          `receipt_merkle:${merkle} f1_score:${ev.f1_score ?? ''}`,
          'public',
          tool,
          merkle,
        );
